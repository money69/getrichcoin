...
func ExampleMarshalCmd() {
	...
	// optional fields.  Also, notice the call to grhjson.Bool which is a
	// convenience function for creating a pointer out of a primitive for
	// optional parameters.
	blockHash := "000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f"
	gbCmd := grhjson.NewGetBlockCmd(blockHash, grhjson.Bool(false), nil)
	...
	id := 1
	marshalledBytes, err := grhjson.MarshalCmd(id, gbCmd)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleUnmarshalCmd() {
	...
	gbCmd, ok := cmd.(*grhjson.GetBlockCmd)
	if !ok {
...
func ExampleMarshalResponse() {
	...
	marshalledBytes, err := grhjson.MarshalResponse(1, 350001, nil)
	if err != nil {
		fmt.Println(err)
		return
	}
