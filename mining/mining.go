...
const (
	...
	MinHighPriority = grhutil.SatoshiPerGetRichCoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr grhutil.Address) (*grhutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *grhutil.Tx, height int32) error {
...
func logSkippedDeps(tx *grhutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress grhutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*grhutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := grhutil.NewTx(coinbaseTx.MsgTx().Copy())
	...
	block := grhutil.NewBlock(&msgBlock)
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A grhutil.Block should use saved in the state to avoid
