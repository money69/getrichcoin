...
type Config struct {
	...
	ProcessBlock func(*grhutil.Block, blockchain.BehaviorFlags) (bool, error)
...
func (m *CPUMiner) submitBlock(block *grhutil.Block) bool {
	...
	log.Infof("Block submitted via CPU miner accepted (hash %s, "+
		"amount %v)", block.Hash(), grhutil.Amount(coinbaseTx.Value))
	return true
}
...
out:
	for {
		...
		if m.solveBlock(template.Block, curHeight+1, ticker, quit) {
			block := grhutil.NewBlock(template.Block)
			m.submitBlock(block)
		}
	}
