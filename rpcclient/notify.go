...
type notificationState struct {
	...
	notifySpent        map[grhjson.OutPoint]struct{}
}
...
func (s *notificationState) Copy() *notificationState {
	...
	stateCopy.notifySpent = make(map[grhjson.OutPoint]struct{})
...
func newNotificationState() *notificationState {
	return &notificationState{
		...
		notifySpent:    make(map[grhjson.OutPoint]struct{}),
	}
}
...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *grhutil.Tx, details *grhjson.BlockDetails)
	...
	//
	// NOTE: Deprecated. Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *grhutil.Tx, details *grhjson.BlockDetails)
	...
	// result of a rescan request, due to how grhd may send various rescan
	...
	// such as grhwallet.
	OnAccountBalance func(account string, balance grhutil.Amount, confirmed bool)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	switch ntfn.Method {
	// OnBlockConnected
	case grhjson.BlockConnectedNtfnMethod:
	...
	case grhjson.FilteredBlockConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnBlockDisconnected
	case grhjson.BlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case grhjson.FilteredBlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRedeemingTx
	case grhjson.RedeemingTxNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case grhjson.RelevantTxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRescanFinished
	case grhjson.RescanFinishedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRescanProgress
	case grhjson.RescanProgressNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAccepted
	case grhjson.TxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAcceptedVerbose
	case grhjson.TxAcceptedVerboseNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnBtcdConnected
	case grhjson.BtcdConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
		...
		if err != nil {
			log.Warnf("Received invalid grhd connected "+
				"notification: %v", err)
			return
		}
	...
	// OnAccountBalance
	case grhjson.AccountBalanceNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWalletLockState
	case grhjson.WalletLockStateNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
...
//
// NOTE: This is a grhd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*grhutil.Tx, error) {
	...
	transactions := make([]*grhutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = grhutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*grhutil.Tx,
	*grhjson.BlockDetails, error) {
	...
	// JSON object.
	var block *grhjson.BlockDetails
	if len(params) > 1 {
	...
	return grhutil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := grhutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
func parseTxAcceptedVerboseNtfnParams(params []json.RawMessage) (*grhjson.TxRawResult,
	error) {
...
// parseBtcdConnectedNtfnParams parses out the connection status of grhd
// and grhwallet from the parameters of a grhdconnected notification.
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance grhutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := grhutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
//
// NOTE: This is a grhd extension and requires a websocket connection.
func (c *Client) NotifyBlocksAsync() FutureNotifyBlocksResult {
	...
	cmd := grhjson.NewNotifyBlocksCmd()
...
func (c *Client) notifySpentInternal(outpoints []grhjson.OutPoint) FutureNotifySpentResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
	...
	cmd := grhjson.NewNotifySpentCmd(outpoints)
	return c.sendCmd(cmd)
}

// newOutPointFromWire constructs the grhjson representation of a transaction
// outpoint from the wire type.
func newOutPointFromWire(op *wire.OutPoint) grhjson.OutPoint {
	return grhjson.OutPoint{
		Hash:  op.Hash.String(),
		Index: op.Index,
	}
}
...
func (c *Client) NotifySpentAsync(outpoints []*wire.OutPoint) FutureNotifySpentResult {
	...
	ops := make([]grhjson.OutPoint, 0, len(outpoints))
	for _, outpoint := range outpoints {
		ops = append(ops, newOutPointFromWire(outpoint))
	}
	cmd := grhjson.NewNotifySpentCmd(ops)
	return c.sendCmd(cmd)
}
...
func (c *Client) NotifyNewTransactionsAsync(verbose bool) FutureNotifyNewTransactionsResult {
	...
	cmd := grhjson.NewNotifyNewTransactionsCmd(&verbose)
	return c.sendCmd(cmd)
}
...
func (c *Client) notifyReceivedInternal(addresses []string) FutureNotifyReceivedResult {
	...
	cmd := grhjson.NewNotifyReceivedCmd(addresses)
	return c.sendCmd(cmd)
}
...
//
// NOTE: Deprecated. Use LoadTxFilterAsync instead.
func (c *Client) NotifyReceivedAsync(addresses []grhutil.Address) FutureNotifyReceivedResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
	...
	cmd := grhjson.NewNotifyReceivedCmd(addrs)
	return c.sendCmd(cmd)
}
...
//
// NOTE: Deprecated. Use LoadTxFilter instead.
func (c *Client) NotifyReceived(addresses []grhutil.Address) error {
...
func (c *Client) RescanAsync(startBlock *chainhash.Hash,
	...
	cmd := grhjson.NewRescanCmd(startBlockHashStr, addrs, ops, nil)
	return c.sendCmd(cmd)
}
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []grhutil.Address, outpoints []*wire.OutPoint,
	...
	cmd := grhjson.NewRescanCmd(startBlockHashStr, addrs, ops,
		&endBlockHashStr)
	return c.sendCmd(cmd)
}
...
// NOTE: This is a grhd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []grhutil.Address,
	...
	outPointObjects := make([]grhjson.OutPoint, len(outPoints))
	for i := range outPoints {
		outPointObjects[i] = grhjson.OutPoint{
			Hash:  outPoints[i].Hash.String(),
			Index: outPoints[i].Index,
		}
	}

	cmd := grhjson.NewLoadTxFilterCmd(reload, addrStrs, outPointObjects)
	return c.sendCmd(cmd)
}
...
// NOTE: This is a grhd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []grhutil.Address, outPoints []wire.OutPoint) error {
