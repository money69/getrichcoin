...
/*
Package rpcclient implements a websocket-enabled GetRichCoin JSON-RPC client.

Overview

This client provides a robust and easy to use client for interfacing with a
GetRichCoin RPC server that uses a grhd/getrichcoin core compatible GetRichCoin JSON-RPC
API.  This client has been tested with grhd (https://github.com/grhsuite/grhd),
grhwallet (https://github.com/grhsuite/grhwallet), and
getrichcoin core (https://github.com/getrichcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, grhd and
grhwallet provide a websocket interface that is more efficient than the standard
...
grhd or grhwallet by default.  However, configuration options are provided to
fall back to HTTP POST and disable TLS to support talking with inferior getrichcoin
...
In contrast, the websocket-based JSON-RPC interface provided by grhd and
grhwallet only uses a single connection that remains open and allows
...
All notifications provided by grhd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by grhd (and
grhwallet passthrough).  Therefore if you call one of these commands against
...
Also, it is important to realize that grhd intentionally separates the wallet
functionality into a separate process named grhwallet.  This means if you are
connected to the grhd RPC server directly, only the RPCs which are related to
chain services will be available.  Depending on your application, you might only
need chain-related RPCs.  In contrast, grhwallet provides pass through treatment
...
detected by type asserting the error in a *grhjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*grhjson.RPCError); ok {
  		switch jerr.Code {
  		case grhjson.ErrRPCUnimplemented:
...
The following full-blown client examples are in the examples directory:
 ...
 - grhdwebsockets
   ...
   block count
 - grhwalletwebsockets
   Connects to a grhwallet RPC server using TLS-secured websockets, registers
