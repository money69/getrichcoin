...
// Client represents a GetRichCoin RPC client which allows easy access to the
// various RPC methods available on a GetRichCoin RPC server.  Each of the wrapper
...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *grhjson.NotifyBlocksCmd:
		c.ntfnState.notifyBlocks = true

	case *grhjson.NotifyNewTransactionsCmd:
...
type (
	...
	rawResponse struct {
		...
		Error  *grhjson.RPCError `json:"error"`
	}
)
...
// returning an unmarshaled grhjson.RPCError (or an unmarshaling error) if so.
...
func (c *Client) reregisterNtfns() error {
	...
	if nslen > 0 {
		outpoints := make([]grhjson.OutPoint, 0, nslen)
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := grhjson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := grhjson.MarshalCmd(id, cmd)
	if err != nil {
		return newFutureError(err)
	}
