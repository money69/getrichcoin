...
/*
Package peer provides a common base for creating and managing GetRichCoin network
peers.

Overview

This package builds upon the wire package, which provides the fundamental
primitives necessary to speak the getrichcoin wire protocol, in order to simplify
...
A quick overview of the major features peer provides are as follows:
 ...
 - Provides a basic concurrent safe getrichcoin peer for handling getrichcoin
   communications via the peer-to-peer protocol
 - Full duplex reading and writing of getrichcoin protocol messages
 ...
 - Flexible peer configuration
   ...
   - Ability to register callbacks for handling getrichcoin protocol messages
...
caller to specify things such as the user agent name and version, the getrichcoin
network to use, which services it supports, and callbacks to invoke when getrichcoin
...
In order to do anything useful with a peer, it is necessary to react to getrichcoin
...
For convenience, a callback hook for all of the currently supported getrichcoin
...
function which allows a grhlog.Logger to be specified.  For example, logging at
...
(https://godoc.org/github.com/grhsuite/grhd/wire#hdr-GetRichCoin_Improvement_Proposals)
*/
package peer
