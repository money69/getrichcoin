...
const (
	defaultConfigFilename        = "grhd.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "grhd.log"
	defaultMaxPeers              = 125
	...
	sampleConfigFilename         = "sample-grhd.conf"
	defaultTxIndex               = false
	defaultAddrIndex             = false
)

var (
	defaultHomeDir     = grhutil.AppDataDir("grhd", false)
...
// config defines the configuration options for grhd.
//
...
type config struct {
	...
	RPCQuirks            bool          `long:"rpcquirks" description:"Mirror some JSON-RPC quirks of GetRichCoin Core -- NOTE: Discouraged unless interoperability issues need to be worked around"`
	...
	MinRelayTxFee        float64       `long:"minrelaytxfee" description:"The minimum transaction fee in GRH/kB to be considered a non-zero fee."`
	FreeTxRelayLimit     float64       `long:"limitfreerelay" description:"Limit relay of transactions with no transaction fee to the given amount in thousands of bytes per minute"`
	NoRelayPriority      bool          `long:"norelaypriority" description:"Do not require free or low-fee transactions to have high priority for relaying"`
	MaxOrphanTxs         int           `long:"maxorphantx" description:"Max number of orphan transactions to keep in memory"`
	Generate             bool          `long:"generate" description:"Generate (mine) getrichcoins using the CPU"`
...
//
// The above results in grhd functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	cfg := config{
		...
		MinRelayTxFee:        mempool.DefaultMinRelayTxFee.ToGRH(),
	...
	if cfg.DisableRPC {
		grhdLog.Infof("RPC service is disabled")
	}
	...
	cfg.minRelayTxFee, err = grhutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]grhutil.Address, 0, len(cfg.MiningAddrs))
	for _, strAddr := range cfg.MiningAddrs {
		addr, err := grhutil.DecodeAddress(strAddr, activeNetParams.Params)
		if err != nil {
	...
	if configFileError != nil {
		grhdLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-grhd.conf to the given destination path,
...
// grhdDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func grhdDial(addr net.Addr) (net.Conn, error) {
...
// grhdLookup resolves the IP of the given host using the correct DNS lookup
...
func grhdLookup(host string) ([]net.IP, error) {
