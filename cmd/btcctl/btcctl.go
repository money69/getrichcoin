...
func main() {
	...
	method := args[0]
	usageFlags, err := grhjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := grhjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// grhjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(grhjson.Error); ok {
		...
		// The error is not a grhjson.Error and this really should not
	...
	marshalledJSON, err := grhjson.MarshalCmd(1, cmd)
	if err != nil {
