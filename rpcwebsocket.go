...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *grhutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *grhutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *grhutil.Tx, isNew bool) {
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) addAddress(a grhutil.Address) {
	switch a := a.(type) {
	case *grhutil.AddressPubKeyHash:
		f.pubKeyHashes[*a.Hash160()] = struct{}{}
		return
	case *grhutil.AddressScriptHash:
		f.scriptHashes[*a.Hash160()] = struct{}{}
		return
	case *grhutil.AddressPubKey:
...
func (f *wsClientFilter) addAddressStr(s string, params *chaincfg.Params) {
	...
	// script.
	a, err := grhutil.DecodeAddress(s, params)
	if err != nil {
		return
	}
	f.addAddress(a)
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) existsAddress(a grhutil.Address) bool {
	switch a := a.(type) {
	case *grhutil.AddressPubKeyHash:
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) removeAddress(a grhutil.Address) {
	switch a := a.(type) {
	case *grhutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected grhutil.Block
type notificationBlockDisconnected grhutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *grhutil.Tx,
...
func (*wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := grhjson.NewBlockConnectedNtfn(block.Hash().String(), block.Height(),
		block.MsgBlock().Header.Timestamp.Unix())
	marshalledJSON, err := grhjson.MarshalCmd(nil, ntfn)
	if err != nil {
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *grhutil.Block) {
	...
	ntfn := grhjson.NewBlockDisconnectedNtfn(block.Hash().String(),
...
func (m *wsNotificationManager) notifyFilteredBlockConnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := grhjson.NewFilteredBlockConnectedNtfn(block.Height(),
...
func (*wsNotificationManager) notifyFilteredBlockDisconnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := grhjson.NewFilteredBlockDisconnectedNtfn(block.Height(),
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *grhutil.Tx) {
	...
	ntfn := grhjson.NewTxAcceptedNtfn(txHashStr, grhutil.Amount(amount).ToGRH())
	marshalledJSON, err := grhjson.MarshalCmd(nil, ntfn)
	if err != nil {
	...
	var verboseNtfn *grhjson.TxAcceptedVerboseNtfn
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			verboseNtfn = grhjson.NewTxAcceptedVerboseNtfn(*rawTx)
			marshalledJSONVerbose, err = grhjson.MarshalCmd(nil,
				verboseNtfn)
			if err != nil {
...
// blockDetails creates a BlockDetails struct to include in grhws notifications
// from a block and a transaction's block index.
func blockDetails(block *grhutil.Block, txIndex int) *grhjson.BlockDetails {
	if block == nil {
		return nil
	}
	return &grhjson.BlockDetails{
		Height: block.Height(),
...
func newRedeemingTxNotification(txHex string, index int, block *grhutil.Block) ([]byte, error) {
	// Create and marshal the notification.
	ntfn := grhjson.NewRedeemingTxNtfn(txHex, blockDetails(block, index))
	return grhjson.MarshalCmd(nil, ntfn)
}
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *grhutil.Tx, block *grhutil.Block) {
	...
	for i, txOut := range tx.MsgTx().TxOut {
		...
		for _, txAddr := range txAddrs {
			...
			ntfn := grhjson.NewRecvTxNtfn(txHex, blockDetails(block,
				tx.Index()))
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *grhutil.Tx,
	clients map[chan struct{}]*wsClient) {

	clientsToNotify := m.subscribedClients(tx, clients)

	if len(clientsToNotify) != 0 {
		n := grhjson.NewRelevantTxAcceptedNtfn(txHexString(tx.MsgTx()))
		marshalled, err := grhjson.MarshalCmd(nil, n)
		if err != nil {
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *grhutil.Tx, block *grhutil.Block) {
...
out:
	for {
		...
		if err != nil {
			...
			jsonErr := &grhjson.RPCError{
				Code:    grhjson.ErrRPCParse.Code,
		...
		//
		// GetRichCoin Core serves requests with "id":null or even an absent "id",
		...
		switch authCmd, ok := cmd.cmd.(*grhjson.AuthenticateCmd); {
		...
		if !c.isAdmin {
			if _, ok := rpcLimited[request.Method]; !ok {
				jsonErr := &grhjson.RPCError{
					Code:    grhjson.ErrRPCInvalidParams.Code,
...
func handleWebsocketHelp(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*grhjson.HelpCmd)
	if !ok {
		return nil, grhjson.ErrRPCInternal
	}
	...
	if !valid {
		return nil, &grhjson.RPCError{
			Code:    grhjson.ErrRPCInvalidParameter,
...
func handleLoadTxFilter(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*grhjson.LoadTxFilterCmd)
...
func handleSession(wsc *wsClient, icmd interface{}) (interface{}, error) {
	return &grhjson.SessionResult{SessionID: wsc.sessionID}, nil
}
...
func handleNotifySpent(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*grhjson.NotifySpentCmd)
	if !ok {
		return nil, grhjson.ErrRPCInternal
	}
...
func handleNotifyNewTransactions(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*grhjson.NotifyNewTransactionsCmd)
	if !ok {
		return nil, grhjson.ErrRPCInternal
	}
...
func handleNotifyReceived(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*grhjson.NotifyReceivedCmd)
	if !ok {
		return nil, grhjson.ErrRPCInternal
	}
...
func handleStopNotifySpent(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*grhjson.StopNotifySpentCmd)
	if !ok {
		return nil, grhjson.ErrRPCInternal
	}
...
func handleStopNotifyReceived(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*grhjson.StopNotifyReceivedCmd)
	if !ok {
		return nil, grhjson.ErrRPCInternal
	}
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		_, err := grhutil.DecodeAddress(addr, params)
		if err != nil {
			return &grhjson.RPCError{
				Code: grhjson.ErrRPCInvalidAddressOrKey,
...
func deserializeOutpoints(serializedOuts []grhjson.OutPoint) ([]*wire.OutPoint, error) {
...
var ErrRescanReorg = grhjson.RPCError{
	Code:    grhjson.ErrRPCDatabase,
	Message: "Reorganize",
}

// rescanBlock rescans all transactions in a single block.  This is a helper
// function for handleRescan.
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *grhutil.Block) {
	for _, tx := range blk.Transactions() {
		...
		for txOutIdx, txout := range tx.MsgTx().TxOut {
			...
			for _, addr := range addrs {
				...
				ntfn := grhjson.NewRecvTxNtfn(txHex,
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *grhutil.Block, params *chaincfg.Params) []string {
...
func handleRescanBlocks(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*grhjson.RescanBlocksCmd)
	if !ok {
		return nil, grhjson.ErrRPCInternal
	}
	...
	discoveredData := make([]grhjson.RescannedBlock, 0, len(blockHashes))
	...
	for i := range blockHashes {
		...
		if err != nil {
			return nil, &grhjson.RPCError{
				Code:    grhjson.ErrRPCBlockNotFound,
		...
		if len(transactions) != 0 {
			discoveredData = append(discoveredData, grhjson.RescannedBlock{
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *grhutil.Block) error {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	...
	for _, addrStr := range cmd.Addresses {
		addr, err := grhutil.DecodeAddress(addrStr, params)
		if err != nil {
			jsonErr := grhjson.RPCError{
				Code: grhjson.ErrRPCInvalidAddressOrKey,
...
fetchRange:
	...
	loopHashList:
		for i := range hashList {
			...
			n := grhjson.NewRescanProgressNtfn(hashList[i].String(),
				blk.Height(), blk.MsgBlock().Header.Timestamp.Unix())
			mn, err := grhjson.MarshalCmd(nil, n)
			if err != nil {
	...
	// Notify websocket client of the finished rescan.  Due to how grhd
	...
	// been sent.
	n := grhjson.NewRescanFinishedNtfn(lastBlockHash.String(),
		lastBlock.Height(),
		lastBlock.MsgBlock().Header.Timestamp.Unix())
	if mn, err := grhjson.MarshalCmd(nil, n); err != nil {
