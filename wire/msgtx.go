...
// OutPoint defines a getrichcoin data type that is used to track previous
// transaction outputs.
...
// NewOutPoint returns a new getrichcoin transaction outpoint point with the
...
// TxIn defines a getrichcoin transaction input.
type TxIn struct {
...
// NewTxIn returns a new getrichcoin transaction input with the provided
...
// TxOut defines a getrichcoin transaction output.
type TxOut struct {
	Value    int64
	PkScript []byte
}
...
// NewTxOut returns a new getrichcoin transaction output with the provided
...
// MsgTx implements the Message interface and represents a getrichcoin tx message.
...
// in that BtcDecode decodes from the getrichcoin wire protocol as it was sent
...
// encodes the transaction to the getrichcoin wire protocol in order to be sent
// across the network.  The wire encoding can technically differ depending on
// the protocol version and doesn't even really need to match the format of a
// stored transaction at all.  As of the time this comment was written, the
// encoded transaction is the same in both instances, but there is a distinct
...
// NewMsgTx returns a new getrichcoin tx message that conforms to the Message
...
// writeOutPoint encodes op to the getrichcoin protocol encoding for an OutPoint
// to w.
...
// writeTxIn encodes ti to the getrichcoin protocol encoding for a transaction
// input (TxIn) to w.
...
// WriteTxOut encodes to into the getrichcoin protocol encoding for a transaction
// output (TxOut) to w.
//
...
// writeTxWitness encodes the getrichcoin protocol encoding for a transaction
