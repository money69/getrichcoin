...
func makeHeader(grhnet GetRichCoinNet, command string,
	payloadLen uint32, checksum uint32) []byte {

	// The length of a getrichcoin message header is 24 bytes.
	// 4 byte magic number of the getrichcoin network + 12 byte command + 4 byte
	// payload length + 4 byte checksum.
	buf := make([]byte, 24)
	binary.LittleEndian.PutUint32(buf, uint32(grhnet))
...
func TestMessage(t *testing.T) {
	...
	tests := []struct {
		...
		grhnet GetRichCoinNet // Network to use for wire encoding
	...
	for i, test := range tests {
		...
		nw, err := WriteMessageN(&buf, test.in, test.pver, test.grhnet)
		if err != nil {
		...
		nr, msg, _, err := ReadMessageN(rbuf, test.pver, test.grhnet)
		if err != nil {
	...
	for i, test := range tests {
		// Encode to wire format.
		var buf bytes.Buffer
		err := WriteMessage(&buf, test.in, test.pver, test.grhnet)
		if err != nil {
			t.Errorf("WriteMessage #%d error %v", i, err)
			continue
		}
		...
		// Decode from wire format.
		rbuf := bytes.NewReader(buf.Bytes())
		msg, _, err := ReadMessage(rbuf, test.pver, test.grhnet)
		if err != nil {
			t.Errorf("ReadMessage #%d error %v, msg %v", i, err,
				spew.Sdump(msg))
			continue
...
func TestReadMessageWireErrors(t *testing.T) {
	...
	// otherwise accurate.
	badMessageBytes := makeHeader(grhnet, "addr", 1, 0xeaadc31c)
	...
	discardBytes := makeHeader(grhnet, "bogus", 15*1024, 0)

	tests := []struct {
		buf     []byte     // Wire encoding
		pver    uint32     // Protocol version for wire encoding
		grhnet  GetRichCoinNet // GetRichCoin network for wire encoding
	...
	for i, test := range tests {
		...
		nr, _, _, err := ReadMessageN(r, test.pver, test.grhnet)
...
func TestWriteMessageWireErrors(t *testing.T) {
	...
	tests := []struct {
		...
		// Command too long.
		{badCommandMsg, pver, grhnet, 0, wireErr, 0},
		// Force error in payload encode.
		{encodeErrMsg, pver, grhnet, 0, wireErr, 0},
		// Force error due to exceeding max overall message payload size.
		{exceedOverallPayloadErrMsg, pver, grhnet, 0, wireErr, 0},
		// Force error due to exceeding max payload for message type.
		{exceedPayloadErrMsg, pver, grhnet, 0, wireErr, 0},
		// Force error in header write.
		{bogusMsg, pver, grhnet, 0, io.ErrShortWrite, 0},
		// Force error in payload write.
		{bogusMsg, pver, grhnet, 24, io.ErrShortWrite, 24},
	}
	...
	for i, test := range tests {
		...
		nw, err := WriteMessageN(w, test.msg, test.pver, test.grhnet)
