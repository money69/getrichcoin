...
import (
	...
	peerpkg "github.com/grhsuite/grhd/peer"
...
// blockMsg packages a getrichcoin block message and the peer it came from together
...
// invMsg packages a getrichcoin inv message and the peer it came from together
// so the block handler has access to that information.
type invMsg struct {
	inv  *wire.MsgInv
	peer *peerpkg.Peer
...
// headersMsg packages a getrichcoin headers message and the peer it came from
...
// txMsg packages a getrichcoin tx message and the peer it came from together
// so the block handler has access to that information.
type txMsg struct {
	tx    *grhutil.Tx
	peer  *peerpkg.Peer
...
func (sm *SyncManager) handleTxMsg(tmsg *txMsg) {
	...
	// NOTE:  GetRichCoinJ, and possibly other wallets, don't follow the spec of
...
func (sm *SyncManager) handleBlockchainNotification(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockAccepted:
		...
		block, ok := notification.Data.(*grhutil.Block)
		if !ok {
...
// processed.
func (sm *SyncManager) QueueTx(tx *grhutil.Tx, peer *peerpkg.Peer, done chan struct{}) {
...
// processed.
func (sm *SyncManager) QueueBlock(block *grhutil.Block, peer *peerpkg.Peer, done chan struct{}) {
...
// chain.
func (sm *SyncManager) ProcessBlock(block *grhutil.Block, flags blockchain.BehaviorFlags) (bool, error) {
