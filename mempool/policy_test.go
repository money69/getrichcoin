...
func TestCalcMinRequiredTxRelayFee(t *testing.T) {
	tests := []struct {
		...
		relayFee grhutil.Amount // minimum relay transaction fee.
...
func TestCheckPkScriptStandard(t *testing.T) {
	var pubKeys [][]byte
	for i := 0; i < 4; i++ {
		pk, err := grhec.NewPrivateKey(grhec.S256())
		if err != nil {
...
func TestDust(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wire.TxOut{Value: grhutil.MaxSatoshi, PkScript: pkScript},
			grhutil.MaxSatoshi,
			false,
		},
		{
...
func TestCheckTransactionStandard(t *testing.T) {
	...
	addr, err := grhutil.NewAddressPubKeyHash(addrHash[:],
	...
	for _, test := range tests {
		...
		err := checkTransactionStandard(grhutil.NewTx(&test.tx),
