...
type Config struct {
	...
	FetchUtxoView func(*grhutil.Tx) (*blockchain.UtxoViewpoint, error)
	...
	// utxo view.
	CalcSequenceLock func(*grhutil.Tx, *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error)
...
type Policy struct {
	...
	// MinRelayTxFee defines the minimum transaction fee in GRH/kB to be
	// considered a non-zero fee.
	MinRelayTxFee grhutil.Amount
}
...
type TxPool struct {
	...
	orphansByPrev map[wire.OutPoint]map[chainhash.Hash]*grhutil.Tx
	outpoints     map[wire.OutPoint]*grhutil.Tx
...
func (mp *TxPool) removeOrphan(tx *grhutil.Tx, removeRedeemers bool) {
...
func (mp *TxPool) RemoveOrphan(tx *grhutil.Tx) {
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *grhutil.Tx, tag Tag) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		if _, exists := mp.orphansByPrev[txIn.PreviousOutPoint]; !exists {
			mp.orphansByPrev[txIn.PreviousOutPoint] =
				make(map[chainhash.Hash]*grhutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *grhutil.Tx, tag Tag) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeOrphanDoubleSpends(tx *grhutil.Tx) {
	msgTx := tx.MsgTx()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *grhutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
	if removeRedeemers {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *grhutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint, tx *grhutil.Tx, height int32, fee int64) *TxDesc {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *grhutil.Tx) error {
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *grhutil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash) (*grhutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAcceptTransaction(tx *grhutil.Tx, isNew, rateLimit, rejectDupOrphans bool) ([]*chainhash.Hash, *TxDesc, error) {
	txHash := tx.Hash()
	...
	if rateLimit && txFee < minFee {
		...
		// window - matches getrichcoind handling.
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *grhutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, *TxDesc, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(acceptedTx *grhutil.Tx) []*TxDesc {
	...
	for processList.Len() > 0 {
		...
		processItem := firstElement.(*grhutil.Tx)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *grhutil.Tx, allowOrphan, rateLimit bool, tag Tag) ([]*TxDesc, error) {
...
// populated grhjson result.
//
// This function is safe for concurrent access.
func (mp *TxPool) RawMempoolVerbose() map[string]*grhjson.GetRawMempoolVerboseResult {
	mp.mtx.RLock()
	defer mp.mtx.RUnlock()

	result := make(map[string]*grhjson.GetRawMempoolVerboseResult,
		len(mp.pool))
	bestHeight := mp.cfg.BestHeight()
	...
	for _, desc := range mp.pool {
		...
		mpd := &grhjson.GetRawMempoolVerboseResult{
			Size:             int32(tx.MsgTx().SerializeSize()),
			Vsize:            int32(GetTxVirtualSize(tx)),
			Fee:              grhutil.Amount(desc.Fee).ToGRH(),
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphansByPrev:  make(map[wire.OutPoint]map[chainhash.Hash]*grhutil.Tx),
		nextExpireScan: time.Now().Add(orphanExpireScanInterval),
		outpoints:      make(map[wire.OutPoint]*grhutil.Tx),
	}
}
