...
const (
	...
	DefaultMinRelayTxFee = grhutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee grhutil.Amount) int64 {
	...
	if minFee < 0 || minFee > grhutil.MaxSatoshi {
		minFee = grhutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *grhutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee grhutil.Amount) bool {
...
func checkTransactionStandard(tx *grhutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee grhutil.Amount,
...
func GetTxVirtualSize(tx *grhutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
