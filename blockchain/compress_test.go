...
func TestAmountCompression(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "0 GRH (sometimes used in nulldata)",
			uncompressed: 0,
			compressed:   0,
		},
		{
		...
		{
			name:         "0.00001 GRH (typical transaction fee)",
			uncompressed: 1000,
			compressed:   4,
		},
		{
			name:         "0.0001 GRH (typical transaction fee)",
			uncompressed: 10000,
			compressed:   5,
		},
		{
		...
		{
			name:         "21000000 GRH (max minted coins)",
...
func TestCompressedTxOut(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "pay-to-pubkey uncompressed 1 GRH",
			amount:       100000000,
			compAmount:   9,
