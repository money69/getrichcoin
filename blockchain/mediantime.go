...
// the time offset mechanism in GetRichCoin Core.  This is necessary because it is
...
func (m *medianTime) AddTimeSample(sourceID string, timeVal time.Time) {
	...
	// buggy behavior in GetRichCoin Core since the median time is used in the
	// consensus rules.
	//
		...
		if !m.invalidTimeChecked {
			...
			if !remoteHasCloseTime {
				log.Warnf("Please check your date and time " +
					"are correct!  grhd will not work " +
