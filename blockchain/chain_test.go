...
func TestHaveBlock(t *testing.T) {
	...
	_, isOrphan, err := chain.ProcessBlock(grhutil.NewBlock(&Block100000),
		BFNone)
	if err != nil {
...
func TestCalcSequenceLock(t *testing.T) {
	...
	// age of 4 blocks.
	targetTx := grhutil.NewTx(&wire.MsgTx{
		TxOut: []*wire.TxOut{{
			PkScript: nil,
			Value:    10,
		}},
	...
	utxoView.AddTxOuts(grhutil.NewTx(unConfTx), 0x7fffffff)
