...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: grhutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func makeTestGenerator(params *chaincfg.Params) (testGenerator, error) {
	privKey, _ := grhec.PrivKeyFromBytes(grhec.S256(), []byte{0x01})
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := grhutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*grhutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, grhutil.NewTx(tx))
	}
...
func additionalCoinbase(amount grhutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee grhutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee grhutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee grhutil.Amount) *wire.MsgTx {
