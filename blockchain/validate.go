...
const (
	...
	baseSubsidy = 50 * grhutil.SatoshiPerGetRichCoin
)
...
func IsCoinBase(tx *grhutil.Tx) bool {
...
func IsFinalizedTransaction(tx *grhutil.Tx, blockHeight int32, blockTime time.Time) bool {
	msgTx := tx.MsgTx()
...
func CheckTransactionSanity(tx *grhutil.Tx) error {
	...
	// as a satoshi.  One getrichcoin is a quantity of satoshi as defined by the
	// SatoshiPerGetRichCoin constant.
	...
	for _, txOut := range msgTx.TxOut {
		...
		// is detected and reported.  This is impossible for GetRichCoin, but
		...
		if totalSatoshi > grhutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
// difficulty is in min/max range and that the block hash is less than the
// target difficulty as claimed.
func CheckProofOfWork(block *grhutil.Block, powLimit *big.Int) error {
...
// txscript.
func CountSigOps(tx *grhutil.Tx) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *grhutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func checkBlockSanity(block *grhutil.Block, powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags) error {
	...
	// checks.  GetRichCoind builds the tree here and checks the merkle root
...
// CheckBlockSanity performs some preliminary checks on a block to ensure it is
// sane before continuing with block processing.  These checks are context free.
func CheckBlockSanity(block *grhutil.Block, powLimit *big.Int, timeSource MedianTimeSource) error {
...
func ExtractCoinbaseHeight(coinbaseTx *grhutil.Tx) (int32, error) {
...
func checkSerializedHeight(coinbaseTx *grhutil.Tx, wantHeight int32) error {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkBlockContext(block *grhutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
...
//
// For more details, see https://en.getrichcoin.it/wiki/BIP_0030 and
// http://r6.ca/blog/20120206T005236Z.html.
//
// This function MUST be called with the chain state lock held (for reads).
func (b *BlockChain) checkBIP0030(node *blockNode, block *grhutil.Block, view *UtxoViewpoint) error {
...
// the getrichcoins and therefore allowed to spend them.  As it checks the inputs,
// it also calculates the total fees for the transaction and returns that value.
//
// NOTE: The transaction MUST have already been sanity checked with the
// CheckTransactionSanity function prior to calling this function.
func CheckTransactionInputs(tx *grhutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		// getrichcoin is a quantity of satoshi as defined by the
		// SatoshiPerGetRichCoin constant.
		originTxSatoshi := utxoEntry.AmountByIndex(originTxIndex)
		if originTxSatoshi < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", grhutil.Amount(originTxSatoshi))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxSatoshi > grhutil.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				grhutil.Amount(originTxSatoshi),
				grhutil.MaxSatoshi)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalSatoshiIn < lastSatoshiIn ||
			totalSatoshiIn > grhutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
	...
	// NOTE: getrichcoind checks if the transaction fees are < 0 here, but that
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *grhutil.Block, view *UtxoViewpoint, stxos *[]spentTxOut) error {
	...
	// https://en.getrichcoin.it/wiki/BIP_0016 for more details.
...
func (b *BlockChain) CheckConnectBlockTemplate(block *grhutil.Block) error {
	b.chainLock.Lock()
