...
grhd provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API that is
fully compatible with the original getrichcoind/getrichcoin-qt.  There are a few key
differences between grhd and getrichcoind as far as how RPCs are serviced:
* Unlike getrichcoind that has the wallet and chain intermingled in the same process
  which leads to several issues, grhd intentionally splits the wallet and chain
  services into independent processes.  See the blog post
  [here](https://blog.conformal.com/grhd-not-your-moms-getrichcoin-daemon/) for
  further details on why they were separated.  This means that if you are
  talking directly to grhd, only chain-related RPCs are available.  However both
  chain-related and wallet-related RPCs are available via
  [grhwallet](https://github.com/grhsuite/grhwallet).
* grhd is secure by default which means that the RPC connection is TLS-enabled
  by default
* grhd provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for grhd RPC and are used by applications
such as [grhwallet](https://github.com/grhsuite/grhwallet) for inter-process
communication with grhd.  The websocket connection endpoint for grhd is
...
during the development of grhwallet.
...
The original getrichcoind/getrichcoin-qt JSON-RPC API documentation is available at [https://en.getrichcoin.it/wiki/Original_GetRichCoin_client/API_Calls_list](https://en.getrichcoin.it/wiki/Original_GetRichCoin_client/API_Calls_list)

<a name="HttpPostVsWebsockets" />

### 2. HTTP POST Versus Websockets

The grhd RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
grhd is `wss://your_ip_or_domain:8334/ws`.
...
to a grhd RPC server:

* **rpcuser** is the full-access username configured for the grhd RPC server
* **rpcpass** is the full-access password configured for the grhd RPC server
* **rpclimituser** is the limited username configured for the grhd RPC server
* **rpclimitpass** is the limited password configured for the grhd RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the grhd
  server is configured with.  It is automatically generated by grhd and placed
  in the grhd home directory (which is typically `%LOCALAPPDATA%\Btcd` on
  Windows and `~/.grhd` on POSIX-like OSes)

**NOTE:** As mentioned above, grhd is secure by default which means the RPC
...
The grhd RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
grhd comes with a separate utility named `grhctl` which can be used to issue
these RPC commands via HTTP POST requests to grhd after configuring it with the
...
API compatible with the original getrichcoind/getrichcoin-qt client.
...
|25|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><font color="orange">grhd does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
|26|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since grhd does not have the wallet integrated to provide payment addresses, grhd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|27|[stop](#stop)|N|Shutdown grhd.|
|28|[submitblock](#submitblock)|Y|Attempts to submit a new serialized, hex-encoded block to the network.|
|29|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Since grhd does not have a wallet integrated, grhd will only return whether the address is valid or not.|
...
|---|---|
|Method|getblock|
|Parameters|1. block hash (string, required) - the hash of the block<br />2. verbose (boolean, optional, default=true) - specifies the block is returned as a JSON object instead of hex-encoded string<br />3. verbosetx (boolean, optional, default=false) - specifies that each transaction is returned as a JSON object and only applies if the `verbose` flag is true.<font color="orange">**This parameter is a grhd extension**</font>|
...
|Notes|<font color="orange">Since grhd does not perform any mining, the priority related fields `startingpriority` and `currentpriority` that are available when the `verbose` flag is set are always 0.</font>|
|Returns (verbose=false)|`[ (json array of string)`<br />&nbsp;&nbsp;`"transactionhash", (string) hash of the transaction`<br />&nbsp;&nbsp;`...`<br />`]`|
|Returns (verbose=true)|`{ (json object)`<br />&nbsp;&nbsp;`"transactionhash": { (json object)`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"size": n, (numeric) transaction size in bytes`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"vsize": n, (numeric) transaction virtual size`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"fee" : n, (numeric) transaction fee in getrichcoins`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"time": n, (numeric) local time transaction entered pool in seconds since 1 Jan 1970 GMT`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"height": n, (numeric) block height when transaction entered the pool`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"startingpriority": n, (numeric) priority when transaction entered the pool`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"currentpriority": n, (numeric) current priority`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"depends": [ (json array) unconfirmed transactions used as inputs for this transaction`<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`"transactionhash", (string) hash of the parent transaction`<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;`...`<br />&nbsp;&nbsp;&nbsp;&nbsp;`]`<br />&nbsp;&nbsp;`}, ...`<br />`}`|
...
|Notes|NOTE: Since grhd does not have the wallet integrated to provide payment addresses, grhd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|<font color="orange">grhd does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown grhd.|
|Returns|`"grhd stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Parameters|1. address (string, required) - getrichcoin address|
|Description|Verify an address is valid.|
|Returns|`{ (json object)`<br />&nbsp;&nbsp;`"isvalid": true or false,  (bool) whether or not the address is valid.`<br />&nbsp;&nbsp;`"address": "getrichcoinaddress", (string) the getrichcoin address validated.`<br />}|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database.<br />The actual checks performed by the `checklevel` parameter is implementation specific.  For grhd this is:<br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|<font color="orange">Btcd currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for grhd.</font>|
...
The following is an overview of the RPC methods which are implemented by grhd, but not the original getrichcoind client. Click the method name for further details such as parameter and return information.
...
|3|[getcurrentnet](#getcurrentnet)|Y|Get getrichcoin network grhd is running on.|None|
...
|Description|Dynamically changes the debug logging level.<br />The levelspec can either a debug level or of the form `<subsystem>=<level>,<subsystem2>=<level2>,...`<br />The valid debug levels are `trace`, `debug`, `info`, `warn`, `error`, and `critical`.<br />The valid subsystems are `AMGR`, `ADXR`, `BCDB`, `BMGR`, `GRHD`, `CHAN`, `DISC`, `PEER`, `RPCS`, `SCRP`, `SRVR`, and `TXMP`.<br />Additionally, the special keyword `show` can be used to get a list of the available subsystems.|
|Returns|string|
|Example Return|`Done.`|
|Example `show` Return|`Supported subsystems [AMGR ADXR BCDB BMGR GRHD CHAN DISC PEER RPCS SCRP SRVR TXMP]`|
...
|Parameters|None|
|Description|Get getrichcoin network grhd is running on.|
|Returns|numeric|
...
|Method|version|
|Parameters|None|
|Description|Returns the version of the JSON-RPC API built into this release of grhd.|
|Returns|`{ (json object)`<br />&nbsp;&nbsp;`"grhdjsonrpcapi": {`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"versionstring": "x.y.z",  (string) the version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"major": x,  (numeric) the major version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"minor": y,  (numeric) the minor version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"patch": z,  (numeric) the patch version of the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"prerelease": "",  (string) prerelease info for the JSON-RPC API`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"buildmetadata": ""  (string) metadata about the server build`<br />&nbsp;&nbsp;`}`<br />`}`|
|Example Return|`{`<br />&nbsp;&nbsp;`"grhdjsonrpcapi": {`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"versionstring": "1.0.0",`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"major": 1,  `<br />&nbsp;&nbsp;&nbsp;&nbsp;`"minor": 0,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"patch": 0,`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"prerelease": "",`<br />&nbsp;&nbsp;&nbsp;&nbsp;`"buildmetadata": ""`<br />&nbsp;&nbsp;`}`<br />`}`|
[Return to Overview](#MethodOverview)<br />
...
|Parameters|1. Addresses (JSON array, required)<br />&nbsp;`[ (json array of strings)`<br />&nbsp;&nbsp;`"getrichcoinaddress", (string) the getrichcoin address`<br />&nbsp;&nbsp;`...`<br />&nbsp;`]`|
...
|Description|*DEPRECATED, for similar functionality see [loadtxfilter](#loadtxfilter)*<br />Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this grhd instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSExtMethodOverview)<br />
...
|Parameters|1. BeginBlock (string, required) block hash to begin rescanning from<br />2. Addresses (JSON array, required)<br />&nbsp;`[ (json array of strings)`<br />&nbsp;&nbsp;`"getrichcoinaddress", (string) the getrichcoin address`<br />&nbsp;&nbsp;`...` <br />&nbsp;`]`<br />3. Outpoints (JSON array, required)<br />&nbsp;`[ (JSON array)`<br />&nbsp;&nbsp;`{ (JSON object)`<br />&nbsp;&nbsp;&nbsp;`"hash":"data", (string) the hex-encoded bytes of the outpoint hash`<br />&nbsp;&nbsp;&nbsp;`"index":n (numeric) the txout index of the outpoint`<br />&nbsp;&nbsp;`},`<br />&nbsp;&nbsp;`...`<br />&nbsp;`]`<br />4. EndBlock (string, optional) hash of final block to rescan|
...
grhd uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll grhd for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
[grhrpcclient](https://github.com/grhsuite/grhrpcclient) package.
...
[grhrpcclient](https://github.com/grhsuite/grhrpcclient) package to connect with
a grhd instance via Websockets, issues [getblockcount](#getblockcount) to
...
import (
	"github.com/grhsuite/grhrpcclient"
...
func main() {
	...
	// generated by grhd when it starts the RPC server and doesn't already
	// have one.
	...
	// exits.
	connCfg := &grhrpcclient.ConnConfig{
		Host:         "localhost:8334",
		Endpoint:     "ws",
		User:         "yourrpcuser",
		Pass:         "yourrpcpass",
	...
	client, err := grhrpcclient.New(connCfg, nil)
	if err != nil {
		log.Fatal(err)
	}
...
The following is an example Go application which uses the
[grhrpcclient](https://github.com/grhsuite/grhrpcclient) package to connect with
a grhd instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[grhrpcclient](https://github.com/grhsuite/grhrpcclient) package to connect with
a grhd instance via Websockets and registers for
...
func main() {
	...
	// notifications.
	ntfnHandlers := grhrpcclient.NotificationHandlers{
	...
	client, err := grhrpcclient.New(connCfg, &ntfnHandlers)
	if err != nil {
		log.Fatal(err)
	}
...
(can be installed with `npm install ws`) to connect with a grhd instance,
...
// generated by grhd when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/grhd/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The grhd generated certificate acts as
