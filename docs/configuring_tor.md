...
grhd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, grhd supports a hybrid,
...
While it is easier to only run as a client, it is more beneficial to the GetRichCoin
...
Configuring grhd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the grhd command
line or in the grhd configuration file.  Typically the Tor proxy address will be
...
By default, grhd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since grhd will act as the server.
...
```text
HiddenServiceDir /var/tor/grhd
HiddenServicePort 8333 127.0.0.1:8333
```

Once Tor is configured to provide the hidden service and you have obtained your
generated .onion address, configuring grhd as a Tor hidden service requires
three flags:
...
```bash
$ ./grhd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
grhd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the grhd command line or in the grhd
...
```bash
$ ./grhd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
grhd provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./grhd --proxy=127.0.0.1:9050 --torisolation
```
