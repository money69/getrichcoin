...
import (
	...
	rpc "github.com/grhsuite/grhd/rpcclient"
	"github.com/grhsuite/grhutil"
)

// nodeConfig contains all the args, and data required to launch a grhd process
...
func newConfig(prefix, certFile, keyFile string, extra []string) (*nodeConfig, error) {
	grhdPath, err := grhdExecutablePath()
	if err != nil {
		return nil, err
	}
...
// arguments returns an array of arguments that be used to launch the grhd
// process.
...
// command returns the exec.Cmd which will be used to start the grhd process.
...
// to the grhd process that is launched via Start().
...
// as the base for the log and data directories for grhd.
...
// start creates a new grhd process, and writes its pid in a file reserved for
...
// stop interrupts the running grhd process process, and waits until it exits
...
// shutdown terminates the running grhd process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := grhutil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
