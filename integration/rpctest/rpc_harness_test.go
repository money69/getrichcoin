...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt grhutil.Amount) *chainhash.Hash {
		...
		// Next, send amt GRH to this address, spending from one of our mature
		// coinbase outputs.
	...
	// input.
	txid := genSpend(grhutil.Amount(5 * grhutil.SatoshiPerGetRichCoin))
	...
	txid = genSpend(grhutil.Amount(500 * grhutil.SatoshiPerGetRichCoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(grhutil.SatoshiPerGetRichCoin, pkScript)

	const numTxns = 5
	txns := make([]*grhutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, grhutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	// The internal wallet of this harness should now have 250 GRH.
	expectedBalance := grhutil.Amount(250 * grhutil.SatoshiPerGetRichCoin)
	...
	// The original wallet should now have a balance of 0 GRH as its entire
	// chain should have been decimated in favor of the main harness'
	// chain.
	expectedBalance = grhutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := grhutil.Amount(50 * grhutil.SatoshiPerGetRichCoin)
	...
	// The current wallet balance should now be at least 50 GRH less
...
func TestHarness(t *testing.T) {
	// We should have (numMatureOutputs * 50 GRH) of mature unspendable
	// outputs.
	expectedBalance := grhutil.Amount(numMatureOutputs * 50 * grhutil.SatoshiPerGetRichCoin)
