...
import (
	...
	"github.com/grhsuite/grhutil/hdkeychain"
)
...
type memWallet struct {
	coinbaseKey  *grhec.PrivateKey
	coinbaseAddr grhutil.Address
	...
	addrs map[uint32]grhutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	// newly generated getrichcoin we can spend.
	addrs := make(map[uint32]grhutil.Address)
...
// SetRPCClient saves the passed rpc connection to grhd as the wallet's
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*grhutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          grhutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (grhutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []grhutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
// fundTx attempts to fund a transaction sending amt getrichcoin. The coins are
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt grhutil.Amount, feeRate grhutil.Amount) error {
	const (
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := grhutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) SendOutputs(outputs []*wire.TxOut,
	feeRate grhutil.Amount) (*chainhash.Hash, error) {
...
//
// This function is safe for concurrent access.
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut, feeRate grhutil.Amount) (*wire.MsgTx, error) {
	m.Lock()
	defer m.Unlock()
	...
	var outputAmt grhutil.Amount
	for _, output := range outputs {
		outputAmt += grhutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() grhutil.Amount {
	m.RLock()
	defer m.RUnlock()

	var balance grhutil.Amount
...
func keyToAddr(key *grhec.PrivateKey, net *chaincfg.Params) (grhutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := grhutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
