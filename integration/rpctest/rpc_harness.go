...
// Harness fully encapsulates an active grhd process to provide a unified
// platform for creating rpc driven integration tests involving grhd. The
// active grhd node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active grhd process is fully
...
func New(activeNet *chaincfg.Params, handlers *rpcclient.NotificationHandlers,
	...
	if handlers.OnFilteredBlockConnected != nil {
		...
		handlers.OnFilteredBlockConnected = func(height int32, header *wire.BlockHeader, filteredTxns []*grhutil.Tx) {
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the grhd node itself. This spawns a new process which will be
	// managed
	...
	// wallet.
	filterAddrs := []grhutil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure grhd properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created grhd
...
func (h *Harness) NewAddress() (grhutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() grhutil.Amount {
...
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut,
	feeRate grhutil.Amount) (*wire.MsgTx, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) GenerateAndSubmitBlock(txns []*grhutil.Tx, blockVersion int32,
	blockTime time.Time) (*grhutil.Block, error) {
	...
	prevBlock := grhutil.NewBlock(mBlock)
...
func baseDir() (string, error) {
	dirPath := filepath.Join(os.TempDir(), "grhd", "rpctest")
