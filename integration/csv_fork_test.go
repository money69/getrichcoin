...
import (
	...
	"github.com/grhsuite/grhd/integration/rpctest"
...
func makeTestOutput(r *rpctest.Harness, t *testing.T,
	amt grhutil.Amount) (*grhec.PrivateKey, *wire.OutPoint, []byte, error) {
	...
	// spend.
	a, err := grhutil.NewAddressPubKey(key.PubKey().SerializeCompressed(), r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0113Activation(t *testing.T) {
	...
	grhdCfg := []string{"--rejectnonstd"}
	r, err := rpctest.New(&chaincfg.SimNetParams, nil, grhdCfg)
	if err != nil {
	...
	const outputValue = grhutil.SatoshiPerGetRichCoin
	...
	txns := []*grhutil.Tx{grhutil.NewTx(tx)}
...
func createCSVOutput(r *rpctest.Harness, t *testing.T,
	numSatoshis grhutil.Amount, timeLock int32,
	...
	p2shAddr, err := grhutil.NewAddressScriptHash(csvScript, r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0068AndBIP0112Activation(t *testing.T) {
	...
	const (
		outputAmt         = grhutil.SatoshiPerGetRichCoin
	...
	for txVersion := int32(0); txVersion < 3; txVersion++ {
		...
		txns := []*grhutil.Tx{grhutil.NewTx(spendingTx)}
		block, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
		if err != nil {
			t.Fatalf("unable to submit block: %v", err)
		}
	...
	for i, test := range tests {
		...
		if !test.accept {
			txns := []*grhutil.Tx{grhutil.NewTx(test.tx)}
			_, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
			if err == nil {
