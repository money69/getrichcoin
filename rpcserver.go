...
var (
	...
	// data.
	gbtCoinbaseAux = &grhjson.GetBlockTemplateResultAux{
...
var (
	...
	ErrRPCUnimplemented = &grhjson.RPCError{
		Code:    grhjson.ErrRPCUnimplemented,
		Message: "Command unimplemented",
	}

	// ErrRPCNoWallet is an error returned to RPC clients when the provided
	// command is recognized as a wallet command.
	ErrRPCNoWallet = &grhjson.RPCError{
		Code:    grhjson.ErrRPCNoWallet,
...
// list of commands that we recognize, but for which grhd has no support because
// it lacks support for wallet functionality. For these commands the user
// should ask a connected instance of grhwallet.
...
// not needed.
func internalRPCError(errStr, context string) *grhjson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return grhjson.NewRPCError(grhjson.ErrRPCInternal.Code, errStr)
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *grhjson.RPCError {
	return grhjson.NewRPCError(grhjson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *grhjson.RPCError {
	return grhjson.NewRPCError(grhjson.ErrRPCNoTxInfo,
...
// These commands will be implemented in grhwallet.
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*grhjson.CreateRawTransactionCmd)
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > grhutil.MaxSatoshi {
			return nil, &grhjson.RPCError{
				Code:    grhjson.ErrRPCType,
				Message: "Invalid amount",
			}
		}

		// Decode the provided address.
		addr, err := grhutil.DecodeAddress(encodedAddr, params)
		if err != nil {
			return nil, &grhjson.RPCError{
		...
		satoshi, err := grhutil.NewAmount(amount)
		if err != nil {
...
// transaction.
func createVinList(mtx *wire.MsgTx) []grhjson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]grhjson.Vin, len(mtx.TxIn))
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.ScriptSig = &grhjson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []grhjson.Vout {
	voutList := make([]grhjson.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var vout grhjson.Vout
		vout.N = uint32(i)
		vout.Value = grhutil.Amount(v.Value).ToGRH()
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	blkHeight int32, chainHeight int32) (*grhjson.TxRawResult, error) {
	...
	txReply := &grhjson.TxRawResult{
		Hex:      mtxHex,
		Txid:     txHash,
		Hash:     mtx.WitnessHash().String(),
		Size:     int32(mtx.SerializeSize()),
		Vsize:    int32(mempool.GetTxVirtualSize(grhutil.NewTx(mtx))),
	...
	if blkHeader != nil {
		// This is not a typo, they are identical in getrichcoind as well.
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*grhjson.DecodeRawTransactionCmd)
	...
	if err != nil {
		return nil, &grhjson.RPCError{
			Code:    grhjson.ErrRPCDeserialization,
			Message: "TX decode failed: " + err.Error(),
		}
	}

	// Create and return the result.
	txReply := grhjson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := grhutil.NewAddressScriptHash(script, s.cfg.ChainParams)
	if err != nil {
	...
	reply := grhjson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleGenerate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if len(cfg.miningAddrs) == 0 {
		return nil, &grhjson.RPCError{
			Code: grhjson.ErrRPCInternal.Code,
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*grhjson.GetAddedNodeInfoCmd)
	...
	if c.Node != nil {
		...
		if !found {
			return nil, &grhjson.RPCError{
				Code:    grhjson.ErrRPCClientNodeNotAdded,
	...
	results := make([]*grhjson.GetAddedNodeInfoResult, 0, len(peers))
	for _, rpcPeer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		peer := rpcPeer.ToPeer()
		var result grhjson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = grhjson.Bool(peer.Connected())
		...
		addrs := make([]grhjson.GetAddedNodeInfoResultAddr, 0, len(ipList))
		for _, ip := range ipList {
			var addr grhjson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBestBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := &grhjson.GetBestBlockResult{
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blk, err := grhutil.NewBlockFromBytes(blkBytes)
	if err != nil {
	...
	blockReply := grhjson.GetBlockVerboseResult{
		Hash:          c.Hash,
	...
	if c.VerboseTx == nil || !*c.VerboseTx {
		...
		rawTxns := make([]grhjson.TxRawResult, len(txns))
		for i, tx := range txns {
...
func handleGetBlockChainInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	chainInfo := &grhjson.GetBlockChainInfoResult{
		Chain:         params.Name,
		...
		Pruned:        false,
		Bip9SoftForks: make(map[string]*grhjson.Bip9SoftForkDescription),
	}
	...
	chainInfo.SoftForks = []*grhjson.SoftForkDescription{
		{
			ID:      "bip34",
			Version: 2,
			Reject: struct {
	...
	for deployment, deploymentDetails := range params.Deployments {
		...
		chainInfo.Bip9SoftForks[forkName] = &grhjson.Bip9SoftForkDescription{
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*grhjson.GetBlockHashCmd)
	hash, err := s.cfg.Chain.BlockHashByHeight(int32(c.Index))
	if err != nil {
		return nil, &grhjson.RPCError{
			Code:    grhjson.ErrRPCOutOfRange,
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*grhjson.GetBlockHeaderCmd)
	...
	params := s.cfg.ChainParams
	blockHeaderReply := grhjson.GetBlockHeaderVerboseResult{
		Hash:          c.Hash,
		Confirmations: uint64(1 + best.Height - blockHeight),
		Height:        blockHeight,
		Version:       blockHeader.Version,
...
// state as a grhjson.GetBlockTemplateResult that is ready to be encoded to JSON
// and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(useCoinbaseValue bool, submitOld *bool) (*grhjson.GetBlockTemplateResult, error) {
	...
	transactions := make([]grhjson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		bTx := grhutil.NewTx(tx)
		resultTx := grhjson.GetBlockTemplateResultTx{
	...
	reply := grhjson.GetBlockTemplateResult{
...
//
// See https://en.getrichcoin.it/wiki/BIP_0022 for more details.
...
func handleGetBlockTemplateRequest(s *rpcServer, request *grhjson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !(cfg.RegressionTest || cfg.SimNet) &&
		...
		return nil, &grhjson.RPCError{
			Code:    grhjson.ErrRPCClientNotConnected,
			Message: "GetRichCoin is not connected",
		}
	}
	...
	if currentHeight != 0 && !s.cfg.SyncMgr.IsCurrent() {
		return nil, &grhjson.RPCError{
			Code:    grhjson.ErrRPCClientInInitialDownload,
			Message: "GetRichCoin is downloading blocks...",
		}
	}
...
// chainErrToGBTErrString converts an error returned from grhchain to a string
...
//
// See https://en.getrichcoin.it/wiki/BIP_0023 for more details.
func handleGetBlockTemplateProposal(s *rpcServer, request *grhjson.TemplateRequest) (interface{}, error) {
	hexData := request.Data
	if hexData == "" {
		return false, &grhjson.RPCError{
			Code: grhjson.ErrRPCType,
...
//
// See https://en.getrichcoin.it/wiki/BIP_0022 and
// https://en.getrichcoin.it/wiki/BIP_0023 for more details.
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*grhjson.GetBlockTemplateCmd)
	request := c.Request
...
func handleGetInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &grhjson.InfoChainResult{
		...
		RelayFee:        cfg.minRelayTxFee.ToGRH(),
	}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &grhjson.GetMempoolInfoResult{
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := grhjson.NewGetNetworkHashPSCmd(nil, nil)
	...
	best := s.cfg.Chain.BestSnapshot()
	result := grhjson.GetMiningInfoResult{
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &grhjson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*grhjson.GetNetworkHashPSCmd)
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*grhjson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.ToPeer().StatsSnapshot()
		info := &grhjson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*grhjson.GetRawMempoolCmd)
	mp := s.cfg.TxMemPool
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*grhjson.GetRawTransactionCmd)
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if includeMempool && s.cfg.TxMemPool.HaveTransaction(txHash) {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &grhjson.RPCError{
				Code: grhjson.ErrRPCInvalidTxVout,
	...
	txOutReply := &grhjson.GetTxOutResult{
		BestBlock:     bestBlockHash,
		Confirmations: int64(confirmations),
		Value:         grhutil.Amount(value).ToGRH(),
		Version:       txVersion,
		ScriptPubKey: grhjson.ScriptPubKeyResult{
			Asm:       disbuf,
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]grhjson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]grhjson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinList[0].Sequence = txIn.Sequence
		return vinList, nil
	}
	...
	vinList := make([]grhjson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &grhjson.PrevOut{
				Addresses: encodedAddrs,
				Value:     grhutil.Amount(originTxOut.Value).ToGRH(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr grhutil.Address, numToSkip, numRequested uint32) ([]*grhutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*grhjson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	params := s.cfg.ChainParams
	addr, err := grhutil.DecodeAddress(c.Address, params)
	if err != nil {
		return nil, &grhjson.RPCError{
			Code:    grhjson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
	...
	// The verbose flag is set, so generate the JSON object and return it.
	best := s.cfg.Chain.BestSnapshot()
	srtList := make([]grhjson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
		...
		if blkHeader != nil {
			// This is not a typo, they are identical in GetRichCoin
			// Core as well.
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*grhjson.SendRawTransactionCmd)
	...
	if err != nil {
		...
		// error code (to match getrichcoind behavior).
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*grhjson.ValidateAddressCmd)

	result := grhjson.ValidateAddressChainResult{}
	addr, err := grhutil.DecodeAddress(c.Address, s.cfg.ChainParams)
	if err != nil {
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*grhjson.VerifyMessageCmd)
	...
	if _, ok := addr.(*grhutil.AddressPubKeyHash); !ok {
		return nil, &grhjson.RPCError{
			Code:    grhjson.ErrRPCType,
	...
	var buf bytes.Buffer
	wire.WriteVarString(&buf, 0, "GetRichCoin Signed Message:\n")
	wire.WriteVarString(&buf, 0, c.Message)
	expectedMessageHash := chainhash.DoubleHashB(buf.Bytes())
	pk, wasCompressed, err := grhec.RecoverCompact(grhec.S256(), sig,
		expectedMessageHash)
	if err != nil {
		// Mirror GetRichCoin Core behavior, which treats error in
	...
	address, err := grhutil.NewAddressPubKey(serializedPK, params)
	if err != nil {
		// Again mirror GetRichCoin Core behavior, which treats error in public key
...
//
// NOTE: This is a grhsuite extension ported from github.com/decred/dcrd.
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	result := map[string]grhjson.VersionResult{
		"grhdjsonrpcapi": {
...
type parsedRPCCmd struct {
	...
	method string
	cmd    interface{}
	err    *grhjson.RPCError
}

// standardCmdResult checks that a parsed command is a standard GetRichCoin JSON-RPC
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, grhjson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *grhjson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(grhjson.Error); ok &&
			jerr.ErrorCode == grhjson.ErrUnregisteredMethod {

			parsedCmd.err = grhjson.ErrRPCMethodNotFound
			return &parsedCmd
		}

		// Otherwise, some type of invalid parameters is the
		// cause, so produce the equivalent RPC error.
		parsedCmd.err = grhjson.NewRPCError(
			grhjson.ErrRPCInvalidParams.Code, err.Error())
		return &parsedCmd
	}
...
// the type *grhjson.RPCError to the appropriate type as needed.
func createMarshalledReply(id, result interface{}, replyErr error) ([]byte, error) {
	var jsonErr *grhjson.RPCError
	if replyErr != nil {
		if jErr, ok := replyErr.(*grhjson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return grhjson.MarshalResponse(id, result, jsonErr)
}
...
func (s *rpcServer) jsonRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	// Terminate with newline to maintain compatibility with GetRichCoin Core.
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="grhd RPC"`)
...
type rpcserverSyncManager interface {
	...
	SubmitBlock(block *grhutil.Block, flags blockchain.BehaviorFlags) (bool, error)
