...
func TestAssignField(t *testing.T) {
	...
	for i, test := range tests {
		...
		err := grhjson.TstAssignField(1, "testField", dst, src)
		if err != nil {
...
func TestNewCmdErrors(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			method: "getblock",
			args:   []interface{}{},
			err:    grhjson.Error{ErrorCode: grhjson.ErrNumParams},
		},
		{
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := grhjson.NewCmd(test.method, test.args...)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
...
func TestMarshalCmdErrors(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := grhjson.MarshalCmd(test.id, test.cmd)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
			t.Errorf("Test #%d (%s) wrong error - got %T (%[2]v), "+
				"want %T", i, test.name, err, test.err)
			continue
...
func TestUnmarshalCmdErrors(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := grhjson.UnmarshalCmd(&test.request)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
			t.Errorf("Test #%d (%s) wrong error - got %T (%[2]v), "+
				"want %T", i, test.name, err, test.err)
			continue
