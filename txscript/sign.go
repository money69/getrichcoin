...
func RawTxInWitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int,
	...
	key *grhec.PrivateKey) ([]byte, error) {
...
// WitnessSignature creates an input witness stack for tx to spend GRH sent
...
func WitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int, amt int64,
	subscript []byte, hashType SigHashType, privKey *grhec.PrivateKey,
	...
	pk := (*grhec.PublicKey)(&privKey.PublicKey)
	var pkData []byte
	if compress {
...
func RawTxInSignature(tx *wire.MsgTx, idx int, subScript []byte,
	hashType SigHashType, key *grhec.PrivateKey) ([]byte, error) {
...
// SignatureScript creates an input signature script for tx to spend GRH sent
...
func SignatureScript(tx *wire.MsgTx, idx int, subscript []byte, hashType SigHashType, privKey *grhec.PrivateKey, compress bool) ([]byte, error) {
...
func p2pkSignatureScript(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType, privKey *grhec.PrivateKey) ([]byte, error) {
	sig, err := RawTxInSignature(tx, idx, subScript, hashType, privKey)
	if err != nil {
		return nil, err
	}
...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []grhutil.Address, nRequired int, kdb KeyDB) ([]byte, bool) {
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	ScriptClass, []grhutil.Address, int, error) {
...
func mergeScripts(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	pkScript []byte, class ScriptClass, addresses []grhutil.Address,
...
func mergeMultiSig(tx *wire.MsgTx, idx int, addresses []grhutil.Address,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		pSig, err := grhec.ParseDERSignature(tSig, grhec.S256())
		if err != nil {
			continue
		}
		...
		for _, addr := range addresses {
			...
			// bad input.
			pkaddr := addr.(*grhutil.AddressPubKey)
...
type KeyDB interface {
	GetKey(grhutil.Address) (*grhec.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(grhutil.Address) (*grhec.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(address grhutil.Address) (*grhec.PrivateKey,
	bool, error) {
	return kc(address)
}
...
type ScriptDB interface {
	GetScript(grhutil.Address) ([]byte, error)
}

// ScriptClosure implements ScriptDB with a closure.
type ScriptClosure func(grhutil.Address) ([]byte, error)

// GetScript implements ScriptDB by returning the result of calling the closure.
func (sc ScriptClosure) GetScript(address grhutil.Address) ([]byte, error) {
	return sc(address)
}
