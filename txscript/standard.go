...
func expectedInputs(pops []parsedOpcode, class ScriptClass) int {
	...
	case MultiSigTy:
		...
		// the original getrichcoind bug where OP_CHECKMULTISIG pops an
...
// specified address.
func PayToAddrScript(addr grhutil.Address) ([]byte, error) {
...
func MultiSigScript(pubkeys []*grhutil.AddressPubKey, nrequired int) ([]byte, error) {
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *chaincfg.Params) (ScriptClass, []grhutil.Address, int, error) {
	var addrs []grhutil.Address
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := grhutil.NewAddressPubKeyHash(pops[2].data,
			chainParams)
		if err == nil {
	...
	case WitnessV0PubKeyHashTy:
		...
		// Skip the pubkey hash if it's invalid for some reason.
		requiredSigs = 1
		addr, err := grhutil.NewAddressWitnessPubKeyHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case PubKeyTy:
		...
		requiredSigs = 1
		addr, err := grhutil.NewAddressPubKey(pops[0].data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := grhutil.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case WitnessV0ScriptHashTy:
		...
		// Skip the script hash if it's invalid for some reason.
		requiredSigs = 1
		addr, err := grhutil.NewAddressWitnessScriptHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]grhutil.Address, 0, numPubKeys)
		for i := 0; i < numPubKeys; i++ {
			addr, err := grhutil.NewAddressPubKey(pops[i+1].data,
				chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
