...
// newAddressPubKey returns a new grhutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) grhutil.Address {
	addr, err := grhutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new grhutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) grhutil.Address {
	addr, err := grhutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new grhutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) grhutil.Address {
	addr, err := grhutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the grhutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the grhutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := grhutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := grhutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := grhutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := grhutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := grhutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*grhutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*grhutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*grhutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*grhutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*grhutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
...
var scriptClassTests = []struct {
	...
	// codeseparator parts have been elided. (getrichcoin core's checks for
