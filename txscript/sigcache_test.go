...
// test data.
func genRandomSig() (*chainhash.Hash, *grhec.Signature, *grhec.PublicKey, error) {
	privKey, err := grhec.NewPrivateKey(grhec.S256())
	if err != nil {
...
func TestSigCacheAddExists(t *testing.T) {
	...
	sig1Copy, _ := grhec.ParseSignature(sig1.Serialize(), grhec.S256())
	key1Copy, _ := grhec.ParsePubKey(key1.SerializeCompressed(), grhec.S256())
...
func TestSigCacheAddEvictEntry(t *testing.T) {
	...
	for i := uint(0); i < sigCacheSize; i++ {
		...
		sigCopy, _ := grhec.ParseSignature(sig.Serialize(), grhec.S256())
		keyCopy, _ := grhec.ParsePubKey(key.SerializeCompressed(), grhec.S256())
	...
	sigNewCopy, _ := grhec.ParseSignature(sigNew.Serialize(), grhec.S256())
	keyNewCopy, _ := grhec.ParsePubKey(keyNew.SerializeCompressed(), grhec.S256())
