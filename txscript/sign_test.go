...
type addressToKey struct {
	key        *grhec.PrivateKey
	compressed bool
}

func mkGetKey(keys map[string]addressToKey) KeyDB {
	if keys == nil {
		return KeyClosure(func(addr grhutil.Address) (*grhec.PrivateKey,
			bool, error) {
...
func mkGetScript(scripts map[string][]byte) ScriptDB {
	if scripts == nil {
		return ScriptClosure(func(addr grhutil.Address) ([]byte, error) {
...
func TestSignTxOutput(t *testing.T) {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			key, err := grhec.NewPrivateKey(grhec.S256())
			if err != nil {
			...
			pk := (*grhec.PublicKey)(&key.PublicKey).
				SerializeUncompressed()
			address, err := grhutil.NewAddressPubKeyHash(
				grhutil.Hash160(pk), &chaincfg.TestNet3Params)
			if err != nil {
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			pk := (*grhec.PublicKey)(&key.PublicKey).
				SerializeUncompressed()
			address, err := grhutil.NewAddressPubKey(pk,
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			scriptAddr, err := grhutil.NewAddressScriptHash(
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			key1, err := grhec.NewPrivateKey(grhec.S256())
			if err != nil {
				t.Errorf("failed to make privKey for %s: %v",
					msg, err)
				break
			...
			pk1 := (*grhec.PublicKey)(&key1.PublicKey).
				SerializeCompressed()
			address1, err := grhutil.NewAddressPubKey(pk1,
				&chaincfg.TestNet3Params)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
			...
			key2, err := grhec.NewPrivateKey(grhec.S256())
			if err != nil {
			...
			pk2 := (*grhec.PublicKey)(&key2.PublicKey).
				SerializeCompressed()
			address2, err := grhutil.NewAddressPubKey(pk2,
				&chaincfg.TestNet3Params)
			if err != nil {
			...
			pkScript, err := MultiSigScript(
				[]*grhutil.AddressPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
...
func TestSignatureScript(t *testing.T) {
	...
	privKey, _ := grhec.PrivKeyFromBytes(grhec.S256(), privKeyD)
