...
// oldBtcdHomeDir returns the OS specific home directory grhd used prior to
// version 0.3.3.  This has since been replaced with grhutil.AppDataDir, but
// this function is still provided for the automatic upgrade path.
func oldBtcdHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "grhd")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".grhd")
	}
...
// location prior to grhd version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to grhd
...
func upgradeDBPaths() error {
	...
	upgradeDBPathNet(filepath.Join(oldDbRoot, "grhd.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "grhd_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "grhd_regtest.db"), "regtest")

	// Remove the old db directory.
	return os.RemoveAll(oldDbRoot)
}

// upgradeDataPaths moves the application data from its location prior to grhd
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		grhdLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old grhd.conf into new location if needed.
		...
		if ohpEmpty {
			...
			grhdLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to grhd as new versions require it.
