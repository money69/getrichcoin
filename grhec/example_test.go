...
func Example_signMessage() {
	...
	privKey, pubKey := grhec.PrivKeyFromBytes(grhec.S256(), pkBytes)
...
func Example_verifySignature() {
	...
	pubKey, err := grhec.ParsePubKey(pubKeyBytes, grhec.S256())
	if err != nil {
		fmt.Println(err)
		return
	}
...
func Example_encryptMessage() {
	...
	message := "test message"
	ciphertext, err := grhec.Encrypt(pubKey, []byte(message))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	privKey, _ := grhec.PrivKeyFromBytes(grhec.S256(), pkBytes)

	// Try decrypting and verify if it's the same message.
	plaintext, err := grhec.Decrypt(privKey, ciphertext)
	if err != nil {
		fmt.Println(err)
		return
	}
